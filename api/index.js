...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === '/groups/create' && req.method === 'POST') {
      if (!body.name || !body.creatorNickname || !body.code) {
        return sendError(res, 'Missing group parameters', 400);
      }
        ...
        members: body.members || [
          {
            ...
            userCode: body.creatorCode || null,
            role: 'owner',
            joinedAt: Date.now(),
          },
        ],
      ...
      return sendJson(res, { status: 'ok', group: groupRecord });
    }

    if (pathname === '/groups/get' && req.method === 'GET') {
      const code = query.code || query.id;
      if (!code) return sendError(res, 'Missing code parameter', 400);
      const supabase = getSupabaseClient();
      if (supabase) {
        try {
          const { data, error } = await supabase
          ...
          if (data && !error) {
            return sendJson(res, {
              group: {
                ...
                avatarUrl: data.avatar_url || null,
                creatorNickname: data.creator_nickname,
                creatorCode: data.creator_code || null,
                membersCount: data.members_count || (data.members ? data.members.length : 1),
    ...
    if (pathname === '/groups/join' && req.method === 'POST') {
      ...
      const userCode = body.userCode || null;
      if (!code || !nickname) return sendError(res, 'Missing code or nickname', 400);
      ...
      if (typeof currentMembers === 'string') {
        try { currentMembers = JSON.parse(currentMembers); } catch { currentMembers = []; }
      }

      const alreadyMember = currentMembers.some((m) => m.nickname === nickname || (userCode && m.userCode === userCode));
      if (!alreadyMember) {
      ...
      const updatedCount = currentMembers.length;
      const groupId = groupData?.id || body.id;
      ...
      const groupResult = {
        id: groupId,
        code: groupData?.code || code,
        name: groupData?.name || 'Group Chat',
        description: groupData?.description || '',
        avatarUrl: groupData?.avatar_url || null,
        creatorNickname: groupData?.creator_nickname || nickname,
        creatorCode: groupData?.creator_code || userCode,
        membersCount: updatedCount,
        maxMembers: 10,
        members: currentMembers,
        createdAt: groupData ? new Date(groupData.created_at).getTime() : Date.now(),
      };

      await triggerPusherEvent(`presence-group-${groupId}`, 'member-joined', {
        groupId,
        members: currentMembers,
        membersCount: updatedCount,
        joinedMember: { nickname, userCode, role: 'member' },
      });

      return sendJson(res, { status: 'ok', group: groupResult });
    }

    if (pathname === '/groups/leave' && req.method === 'POST') {
      const { groupId, nickname, userCode } = body;
      if (!groupId || !nickname) return sendError(res, 'Missing groupId or nickname', 400);

      const supabase = getSupabaseClient();
      let currentMembers = [];
      if (supabase) {
        try {
          const { data } = await supabase.from('groups').select('members').eq('id', groupId).maybeSingle();
          if (data?.members) {
            currentMembers = Array.isArray(data.members) ? data.members : JSON.parse(data.members);
          }
          currentMembers = currentMembers.filter((m) => m.nickname !== nickname && (!userCode || m.userCode !== userCode));
          await supabase.from('groups').update({
            members: currentMembers,
            members_count: currentMembers.length,
          }).eq('id', groupId);
        } catch {}
      }

      await triggerPusherEvent(`presence-group-${groupId}`, 'member-left', {
        groupId,
        nickname,
        userCode,
        members: currentMembers,
      ...
      return sendJson(res, { status: 'ok', membersCount: currentMembers.length });
    }

    if (pathname === '/groups/kick' && req.method === 'POST') {
      const { groupId, targetNickname } = body;
      if (!groupId || !targetNickname) return sendError(res, 'Missing groupId or targetNickname', 400);
      ...
      if (supabase) {
        try {
          ...
          currentMembers = currentMembers.filter((m) => m.nickname !== targetNickname);
          await supabase.from('groups').update({
            members: currentMembers,
            members_count: currentMembers.length,
          }).eq('id', groupId);
      ...
      await triggerPusherEvent(`presence-group-${groupId}`, 'kick', {
        groupId,
