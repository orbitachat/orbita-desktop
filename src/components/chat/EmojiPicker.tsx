    ...
    >
      ...
      >
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {/* TAB 2: STICKERS */}
            {activeTab === 'stickers' && (
              <div className="flex flex-col flex-1 h-full items-center justify-center gap-3">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="80" height="80" className="text-white/30">
                  <g fill="none">
                    <path stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" d="M9 17c.85-.63 1.885-1 3-1s2.15.37 3 1" />
                ...
                <div className="text-white/40 text-sm">Стикеров пока нет</div>
              </div>
            )}
            ...
            {activeTab === 'gif' && (
              gifRows.length === 0 ? (
                <div className="flex flex-col flex-1 h-full items-center justify-center gap-2">
                  {isGifLoading ? (
                    <div className="text-white/40 text-sm animate-pulse">Загрузка GIF...</div>
                  ) : (
                    <div className="text-white/40 text-sm">Гифки не найдены</div>
                  )}
                </div>
              ) : (
                <div
          ...
          {/* Bottom Bar: Tab-Specific Categories & Navigation */}
          {activeTab !== 'emoji' && (
            <div className="h-10 px-2 flex-shrink-0 flex items-center justify-between" style={{ backgroundColor: 'var(--surface-container, rgba(0,0,0,0.15))' }}>
              {activeTab === 'stickers' && (
                <div className="flex items-center gap-2 overflow-x-auto w-full px-1 scrollbar-none">
                  <button
                    type="button"
