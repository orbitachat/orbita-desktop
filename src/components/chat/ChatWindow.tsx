...
import { parseReplyChain, countEmojis, formatTimeOfDay, markdownToHtml, arrayBufferToBase64, formatLastSeen } from '../../utils/messageUtils';
...
const parseMedia = (msg: Message): { type: 'image' | 'video' | 'audio' | 'music' | 'voice' | 'file' | 'call' | 'sticker' | null; url: string | null; fileName?: string; mime?: string } => {
  ...
  const isGifMsg =
    ...
    (msg.mediaUrl && (/\.gif(\?.*)?$/i.test(msg.mediaUrl) || msg.mediaUrl.includes('giphy.com') || msg.mediaUrl.includes('tenor.com')));

  if (isGifMsg) {
    const url = msg.mediaUrl || msg.text?.match(/^\[GIF\]\s*(https?:\/\/[^\s]+)/i)?.[1] || msg.text?.match(/https?:\/\/[^\s]+/)?.[0] || null;
    return { type: 'video', url, fileName: msg.mediaName || 'animation.mp4', mime: msg.mime || 'video/mp4' };
  }

  if (msg.mediaType && msg.mediaUrl) {
    if (msg.mediaType === 'photo') return { type: 'image', url: msg.mediaUrl, fileName: msg.mediaName, mime: msg.mime };
    if (msg.mediaType === 'video') return { type: 'video', url: msg.mediaUrl, fileName: msg.mediaName, mime: msg.mime };
    if (msg.mediaType === 'audio') return { type: 'music', url: msg.mediaUrl, fileName: msg.mediaName, mime: msg.mime };
    if (msg.mediaType === 'file') return { type: 'file', url: msg.mediaUrl, fileName: msg.mediaName, mime: msg.mime };
    if (msg.mediaType === ('sticker' as any)) return { type: 'sticker', url: msg.mediaUrl, fileName: msg.mediaName, mime: msg.mime };
  }

  const text = msg.text || '';
  const stickerMatch = text.match(/^\[Sticker\]\s*(https?:\/\/[^\s]+)/i);
  if (stickerMatch) return { type: 'sticker', url: stickerMatch[1] };
  const gifMatch = text.match(/^\[GIF\]\s*(https?:\/\/[^\s]+)/i);
  if (gifMatch) return { type: 'image', url: gifMatch[1] };
  const imageMatch = text.match(/^\[Photo\]\s*(https?:\/\/[^\s]+\.(jpg|jpeg|png|gif|webp)(\?[^\s]*)?)/i);
  if (imageMatch) return { type: 'image', url: imageMatch[1] };
  const imageMatchGeneric = text.match(/^\[Photo\]\s*(https?:\/\/[^\s]+)/i);
  if (imageMatchGeneric) return { type: 'image', url: imageMatchGeneric[1] };
  const videoMatch = text.match(/^\[Video\]\s*(https?:\/\/[^\s]+\.(mp4|mov|avi|webm|mkv|m4v|3gp)(\?[^\s]*)?)/i);
  if (videoMatch) return { type: 'video', url: videoMatch[1] };
  const videoMatchGeneric = text.match(/^\[Video\]\s*(https?:\/\/[^\s]+)/i);
  if (videoMatchGeneric) return { type: 'video', url: videoMatchGeneric[1] };
  const voiceMatch = text.match(/^\[Audio\]\s+(voice_\S+)\s+(https?:\/\/\S+)\s*$/i);
  if (voiceMatch) return { type: 'voice', url: voiceMatch[2], fileName: voiceMatch[1] };
  const audioMatch = text.match(/^\[Audio\]\s+(.*?)\s+(https?:\/\/\S+)\s*$/i);
  if (audioMatch && !audioMatch[1].match(/^voice_/i) && !audioMatch[1].endsWith('.ogg')) return { type: 'music', url: audioMatch[2], fileName: audioMatch[1] };
  const fileMatch = text.match(/^\[File\]\s+(.*?)\s+(https?:\/\/\S+)\s*$/i);
  if (fileMatch) return { type: 'file', url: fileMatch[2], fileName: fileMatch[1] };
...
interface ContextMenuState {
  ...
  messageId?: string;
  isOwn: boolean;
  type: 'text' | 'singleImage' | 'multipleImages' | 'file';
...
export const ChatWindow = ({ isMobileView = false, onBack }: ChatWindowProps) => {
  ...
  const scrollToPinnedMessage = () => {
    if (pinnedMessage) {
      const index = messages.findIndex(m => m.text === pinnedMessage.text && m.sender === pinnedMessage.sender);
      if (index >= 0) {
  ...
  const triggerPinMessage = (index: number) => {
    ...
    if (activeChatId === 'notes') {
      ...
      updateChat(activeChatId, { pinnedMessage: { sender: pinnedMsg.sender, text: pinnedMsg.text, time: pinnedMsg.time } });
      setContextMenu(prev => ({ ...prev, visible: false }));
      return;
    }
    ...
    const pinnedMsg = messages[index];
    const send = () => {
      channel.trigger('client-message', { sender: myNickname, text: pinnedMsg.text, type: 'pin', pinData: { sender: pinnedMsg.sender, text: pinnedMsg.text, time: pinnedMsg.time } });
      updateChat(activeChatId, { pinnedMessage: { sender: pinnedMsg.sender, text: pinnedMsg.text, time: pinnedMsg.time } });
    };
    if (channel.subscribed) send(); else channel.bind('pusher:subscription_succeeded', send);
    setContextMenu(prev => ({ ...prev, visible: false }));
  ...
  const getMessageMenuType = useCallback((msg: Message): 'text' | 'singleImage' | 'multipleImages' | 'file' => {
    const media = parseMedia(msg);
  ...
  const handleSendFiles = async (caption: string, group: boolean, asFile: boolean) => {
    ...
    const resolveItemType = (ft: string, fileName?: string) => {
      ...
      if (fileName && (/\.gif$/i.test(fileName) || /\.webp$/i.test(fileName))) return 'gif';
    ...
    for (let i = 0; i < attachedFiles.length; i++) {
      ...
      if (file.uploadedUrl) {
        uploadedFiles.push({
          ...
          name: file.name,
          mime: file.fileType === 'audio' ? 'audio/mpeg' : file.fileType === 'photo' ? 'image/jpeg' : file.fileType === 'video' ? 'video/mp4' : 'application/octet-stream',
            ...
            {media.type === 'sticker' && (
              ...
              >
                <img
                  src={media.url!}
                  alt="Стикер"
                  className="w-[170px] h-[170px] object-contain select-none cursor-pointer"
                  style={{ userSelect: 'none' }}
                  loading="lazy"
                />
                <div
                  className="select-none tabular-nums opacity-0 group-hover:opacity-100 transition-opacity duration-150"
                  style={{
                ...
                >
                  ...
                  {isOwn && msg.status && <MessageStatus status={msg.status} />}
                </div>
              </div>
            )}
              ...
              <span
                ...
                dangerouslySetInnerHTML={{
                  __html: markdownToHtml(
                    parseReplyChain(pinnedMessage.text || '').body.trim() || pinnedMessage.text.replace(/^↩\s(?:\[id:.+?\]\s)?.+?:.+?,\s\d{2}:\d{2}\n?/, '').trim(),
                    'var(--accent-color)'
                  ).replace(/<br\s*\/?>/gi, ' ')
                }}
                onClick={(e) => {
