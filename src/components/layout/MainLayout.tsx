...
const getLastMsgDisplay = (chat: Chat, messages: Message[], t: any): React.ReactNode => {
  ...
  if (!chat.lastMsg && messages.length === 0) return t('common.no_messages');
...
const ChatListItem = React.memo(({
  ...
  const messages = useChatStore(useShallow((s) => s.messagesByChatId[chat.id] || EMPTY_CHAT_MESSAGES));
  const lastMsg = messages.length > 0 ? messages[messages.length - 1] : null;
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            >
              {showDraft ? (
                ...
                getLastMsgDisplay(chat, messages, t)
              )}
            </p>
