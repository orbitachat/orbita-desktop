...
export function extractCodeFromInput(input: string): string {
  ...
  const urlMatch = trimmed.match(/\/u\/([a-zA-Z0-9_-]{32,64})/i);
  if (urlMatch && urlMatch[1]) {
    return urlMatch[1];
  }

  const queryMatch = trimmed.match(/[?&](?:code|id)=([a-zA-Z0-9_-]{32,64})/i);
  if (queryMatch && queryMatch[1]) {
    return queryMatch[1];
  }

  const deepLinkMatch = trimmed.match(/orbita:\/\/(?:connect|invite|u)?\/?(?:code=)?([a-zA-Z0-9_-]{32,64})/i);
